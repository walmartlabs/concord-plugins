...
import static com.walmartlabs.concord.sdk.ContextUtils.getWorkDir;
...
public class ConfluenceTask implements Task {
    ...
    public void execute(Context ctx) {
        Result result = new ConfluenceTaskCommon(getWorkDir(ctx), ctx.toMap())
                .execute(TaskParams.of(new ContextVariables(ctx), defaults));

        // createPage:
        ctx.setVariable("result", toMap(result));
    ...
    private static Object toMap(Result result) {
        ObjectMapper om = new ObjectMapper();
        return om.convertValue(result, Map.class);
    }
