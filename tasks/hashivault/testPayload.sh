...
cp target/hashivault-task-*SNAPSHOT.jar target/test/lib/
...
HTTP_CODE=$(curl -sn \
  ...
  -w '%{http_code}' \
  -F org=Default \
  -F arguments.vaultBaseUrl="${VAULT_BASE_URL}" \
  -F arguments.vaultToken="${VAULT_TOKEN}" \
  -F arguments.vaultNs="${VAULT_NS}" \
  -F arguments.vaultPath="${VAULT_PATH}" \
